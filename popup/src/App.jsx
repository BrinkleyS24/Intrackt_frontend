  ...
  if (!isLoggedIn) {
    return (
      <div
        ...
        className="flex h-[600px] max-h-[600px] w-[400px] flex-col overflow-hidden bg-background text-foreground"
      >
        <Notification />

        {/* Scrollable: value first, then the heads-up. The CTA stays pinned below. */}
        <div className="flex-1 overflow-y-auto popup-scrollbar px-5 pt-6">
          <div className="flex flex-col gap-4">
            {/* Brand + value-first headline */}
            <div className="text-center">
              <div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-2xl bg-primary shadow-lg shadow-primary/15">
                <img src={LOGO_URL} alt="" className="h-7 w-7" />
              </div>
              <h1 className="text-[19px] font-bold leading-tight text-foreground">
                Your inbox already tracks
                <br />
                your job search.
              </h1>
            ...
            {/* What you get — value before the ask */}
            <ul className="space-y-2">
              {[
                'Every application sorted by stage: Applied, Interviews, Offers.',
                'Recruiter replies surfaced so nothing gets buried again.',
                'Read-only access. It can never send, delete, or change an email.',
              ].map((line) => (
                <li key={line} className="flex items-start gap-2.5 text-[13px] text-foreground/90">
            ...
            {/* Heads-up: the scary Google screen + how to get past it, before they hit it */}
            <div
            ...
            >
              <p className="flex items-center gap-1.5 text-xs font-semibold text-foreground">
                ...
                Next, Google asks for read-only Gmail access.
              </p>
              <p className="mt-1.5 text-[11.5px] leading-relaxed text-muted-foreground">
                You may see a &ldquo;Google hasn&rsquo;t verified this app&rdquo; notice. We&rsquo;re
                mid-way through Google&rsquo;s independent security audit (CASA); it reflects our
                verification progress, not what the app can access.
              </p>
              <div className="mt-2 rounded-lg border border-accent/25 bg-accent/[0.08] p-2.5">
                <p className="text-[11px] font-semibold text-foreground">
                  To continue past Google&rsquo;s screen:
                </p>
                <ol className="mt-1 space-y-0.5 text-[11px] text-muted-foreground">
                  <li>
                    <span className="font-semibold text-accent">1.</span> Click{' '}
        ...
        {/* Pinned CTA — always visible, no scrolling to find sign-in */}
        <div className="shrink-0 border-t border-white/10 bg-white/[0.02] px-5 py-3">
          <button
            onClick={loginGoogleOAuth}
            disabled={isLoginPending}
            data-testid="login-google-button"
            className={`flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-3 font-semibold shadow-sm transition-colors duration-200 ${
              isLoginPending
          ...
          >
            ...
            <span>{isLoginPending ? 'Signing in...' : 'Continue with Google'}</span>
          </button>
