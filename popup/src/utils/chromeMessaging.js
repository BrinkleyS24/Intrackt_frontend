    ...
    try {
      ...
      if (chrome.runtime.lastError) {
        ...
        if (errorMessage.includes("receiving end") && i < retries - 1) {
          console.warn(`Intrackt: Background script not ready, retrying in ${delay}ms... (Attempt ${i + 1}/${retries})`);
      ...
      if (error.message.includes("receiving end") && i < retries - 1) {
        console.warn(`Intrackt: Failed to connect to background script, retrying... (Attempt ${i + 1}/${retries})`);
        await new Promise(resolve => setTimeout(resolve, delay));
        continue; // Try sending the message again
      }
