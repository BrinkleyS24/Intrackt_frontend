...
export function initializeEventListeners(state, elements, {
    ...
    elements.applyFiltersBtn?.addEventListener("click", () => applyFilters(state, elements, CONFIG));
    elements.clearFiltersBtn?.addEventListener("click", () => clearFilters(state, elements, CONFIG));
    elements.searchBar?.addEventListener("input",
        debounce(() => applyFilters(state, elements, CONFIG), 300)
    );
